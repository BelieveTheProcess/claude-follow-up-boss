...
- `.env` is loaded automatically via the `dotenv` package (`dotenv.config({ quiet: true })` at the top of `src/index.js`) - it silently no-ops if the file doesn't exist (e.g. on Railway, where real env vars are injected directly and always take precedence over anything in a stray `.env`).
...
- The `dealmachine_*` tools talk to DealMachine's public REST API (`https://api.v2.dealmachine.com/v1`) directly, rather than through a Zapier bridge or DealMachine's own hosted MCP connector. That's deliberate: a Zapier-mediated connection has its own separate per-account "tasks" quota on top of DealMachine's normal credits (hit repeatedly during testing, including on calls that don't spend any DealMachine credit at all, e.g. `list_fields`), and neither a Zapier action nor DealMachine's hosted connector can be attached to a *scheduled* Claude session the way tools on this repo's own MCP server can.
- `dealMachineClient.js` and the `dealmachine_*` tools were originally built from DealMachine's published docs (`api.docs.dealmachine.com`) without a live key, then smoke-tested end to end once one was available - several endpoints didn't match the docs and needed real fixes: `dealmachine_location_search` hits `GET /locations`, not `/locations/list` as documented (the documented path 404'd, misread as a lookup for a location literally named "list"); there is no dedicated `/properties/count` or `/dnc-check` endpoint despite being documented - `dealmachine_property_count` now runs a free `per_page:1` search and reads `totals` off the response instead, and `dealmachine_check_dnc` now calls `/enrichment/phone` (same endpoint `dealmachine_enrich_phone` uses) and reads the `do_not_call` flag off the matched phone; and all five `dealmachine_enrich_*` tools send their lookup array under a `data` key (e.g. `{"data":[{"phone":"..."}]}`), not the type-named key the docs implied (`phones`/`apns`/`addresses`/`people`/`emails`). `dealmachine_property_search`, `dealmachine_property_get`, `dealmachine_people_search`, `dealmachine_people_get`, `dealmachine_filters`, `dealmachine_fields`, `dealmachine_usage`, and `dealmachine_comps` (once corrected to `property_ids` as an array, not `property_id`) all matched the docs and worked as built.
